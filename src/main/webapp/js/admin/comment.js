window.onload = function(){
	 ...
	 var div_comment = new Vue({
		 ...
		 data : {
			 ...
			 criteria : {title:'',from_name:'',to_name:'',startTime:'',endTime:''},
		 ...
		 methods: {
			 ...
			 deleteOne : function(i){
				 my.confirm('警告','确定删除此条评论吗？',function(){
					 div_comment.commentList.splice(i, 1);
					 my.show('删除成功!');
				 });
			 },
			 checkAll : function(){
				 ...
				 if(!checked){
					 _this.commentList.forEach(function(item) {
				 ...
				 my.confirm('警告','确定删除选中的评论吗？',function(){
					 ...
					 tool.ajax.post('comment/delete',param,function(res){
					     ...
					     for(var i=0;i<_self.check.selects.length;i=0){
							 for(var j=0;j<_self.commentList.length;j++){
					        	if(_self.commentList[j].id==_self.check.selects[i]){
					        		_self.commentList.splice(j, 1);
